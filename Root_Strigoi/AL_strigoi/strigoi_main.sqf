...
while {alive _strigoi} do 
{
	...
	_tgt_casp = selectRandom ( _list_unit_range_casp select { typeOf _x != "VirtualCurator_F" } );
	...
	while {(!isNil "_tgt_casp") && {(alive _strigoi) && ((_strigoi distance getMarkerPos _poz_orig_sc) < _teritoriu)}} do 
	{
		...
		if ((!alive _tgt_casp)or(_tgt_casp distance getMarkerPos _poz_orig_sc > _teritoriu)) then 
		{
			...
			if !(count _list_unit_range_casp isEqualTo 0) then {_tgt_casp = selectRandom ( _list_unit_range_casp select { typeOf _x != "VirtualCurator_F" })} else {_tgt_casp = nil};
		};
		sleep 1;
	};
	_strigoi call fnc_hide_strig;
