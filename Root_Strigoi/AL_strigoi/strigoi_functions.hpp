...
fnc_attk_strigoi = {
	...
	[[_strigoi,_tgt_casp],"\Root_Anomalies\Root_Strigoi\AL_strigoi\strigoi_atk_viz.sqf"] remoteExec ["execVM"];
	if ((isPlayer _tgt_casp) && (typeOf _tgt_casp != "VirtualCurator_F")) then {
		[[_damage_strig],"\Root_Anomalies\Root_Strigoi\AL_strigoi\strigoi_tgt_attk.sqf"] remoteExec ["execVM",_tgt_casp]
	} else {
		if (_tgt_casp isKindOf "Man") then {
			...
			if (typeOf _tgt_casp != "VirtualCurator_F") then { [_tgt_casp, _damage_strig, _bodyPart, _dmgType] call ace_medical_fnc_addDamageToUnit; } else {};
		} else {
