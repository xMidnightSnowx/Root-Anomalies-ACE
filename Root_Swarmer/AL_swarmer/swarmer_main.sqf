	...
	if (count _list_unit_range_hiv >0) then 
	{
		...
		while {(alive _tgt_hiv)and(_tgt_hiv distance _mobile_s < _radius)} do
		{
			...
			{	
				...
				_amountOfDamage = selectRandom [0.5, 0.6, 0.7, 0.8, 0.9, 1];
				_type_of_damage = selectRandom ["backblast", "bite", "bullet", "explosive", "falling", "grenade", "punch", "ropeburn", "shell", "stab", "unknown", "vehiclecrash"];
				_bodyPart = selectRandom ["head","body","hand_l","hand_r","leg_l","leg_r"];
				[_tgt_hiv, _amountOfDamage, _bodyPart, _type_of_damage] remoteExecCall ["ace_medical_fnc_addDamageToUnit",0];
				};
				{[_mobile_s,_x] spawn fnc_avoid_hive} foreach _list_unit_range_hiv;
				uisleep 3;
				atak_swarmer = false; publicVariable "atak_swarmer";
