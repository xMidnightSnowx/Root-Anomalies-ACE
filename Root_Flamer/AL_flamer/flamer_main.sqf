...
_flamer setVariable ["al_dam_total",_hp_curr_flamer,true]; _flamer setVariable ["al_dam_incr",_hp_curr_flamer,true];

_flamer removeAllEventHandlers "hit"; _flamer addEventHandler ["Hit", {[[_this select 0],"\Root_Anomalies\Root_Flamer\AL_flamer\flamer_splash_hit.sqf"] remoteExec ["execvm"]}];
...
for "_i" from 0 to 5 do {_flamer setObjectMaterialGlobal [_i,_flamer setObjectTextureglobal [_i, "\Root_Anomalies\Root_Flamer\AL_flamer\03_flesh.jpg"]]};
_flamer setVariable ["atk",false];
_flamer call fnc_hide_flamer;
_list_unit_range_flamer = [];
_damage = _damage_flamer;
[_flamer] execvm "\Root_Anomalies\Root_Flamer\AL_flamer\flamer_end.sqf";
while {alive _flamer} do 
{
	...
	while {(!isNil "_tgt_flamer") && {(alive _flamer) && ((_flamer distance getMarkerPos _poz_orig_sc) < _teritoriu)}} do 
	{
		_nearflamer = (ASLToAGL getPosASL _flamer) nearEntities ["CAManBase",5]; 
		{
			...
			_dmgType = selectRandom ["backblast", "explosive", "grenade", "shell", "unknown", "vehiclecrash"];
			if (typeOf _x != "VirtualCurator_F") then 
			{
				[_x, _damage, _bodyPart, _dmgType] call ace_medical_fnc_addDamageToUnit;
		...
		else {_nearflamer = (ASLToAGL getPosASL _flamer) nearEntities ["CAManBase",5]; 
			{
				_bodyPart = ["Head", "RightLeg", "LeftArm", "Body", "LeftLeg", "RightArm"] selectRandomWeighted [0.47,0.69,0.59,0.55,0.61,0.58];
				_dmgType = selectRandom ["backblast", "explosive", "grenade", "shell", "unknown", "vehiclecrash"];
				if (typeOf _x != "VirtualCurator_F") then { [_x, _damage, _bodyPart, _dmgType] call ace_medical_fnc_addDamageToUnit; } else { [_x] call ace_medical_treatment_fnc_fullHealLocal; };
			} forEach (_nearflamer-[_flamer]); [[_flamer],"\Root_Anomalies\Root_Flamer\AL_flamer\flamer_jump_SFX.sqf"] remoteExec ["execvm"]; [_flamer,_tgt_flamer,_cap_flamer] spawn fnc_jump_flamer};
		sleep 1;
		_nearflamer = (ASLToAGL getPosASL _flamer) nearEntities ["CAManBase",5];
		{
			_bodyPart = ["Head", "RightLeg", "LeftArm", "Body", "LeftLeg", "RightArm"] selectRandomWeighted [0.47,0.69,0.59,0.55,0.61,0.58];
