...
fnc_attk_flamer = {
	...
	_nearflamer = (ASLToAGL getPosASL _flamer) nearEntities ["CAManBase",5];
	{
		...
		if (typeOf _x != "VirtualCurator_F") then 
			{
				[_x, _damage_flamer, _bodyPart, _dmgType] call ace_medical_fnc_addDamageToUnit;
				_tip = selectrandom ["04","burned","02","03"];
	...
	_nearvik = nearestObjects [position _flamer,["CAR","TANK","PLANE","HELICOPTER","Motorcycle","Air"],7,false]; {_x setDamage (getdammage _x + ( _damage_flamer * 5 ))} forEach _nearvik;
	sleep 4;
...
fnc_show_flamer = {
	...
	[[_flamer,_damage_flamer],"\Root_Anomalies\Root_Flamer\AL_flamer\flamer_sfx.sqf"] remoteExec ["execVM",0];
