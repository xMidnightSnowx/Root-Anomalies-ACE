...
addCamShake [1, 4, 23];
[[_cap, _coada], "\Root_Anomalies\Root_Worm\AL_worm\worm_attack.sqf"] remoteExec ["execVM", 0];
[[_cap], "\Root_Anomalies\Root_Worm\AL_worm\bump.sqf"] remoteExec ["execVM", 0];
sleep 1;

while {!isNull _cap} do {
    _tgt_worm = [];
    _list_ai_in_range_worm = (position _cap) nearEntities [["Man", "landvehicle"], 150];
    _tgt_worm = selectRandom (_list_ai_in_range_worm select { typeOf _x != "VirtualCurator_F" });
    if (count _list_ai_in_range_worm > 0) then {
            ...
            _nearobj_wrom = nearestobjects [getPosATL _cap, [], 15];
            {
                if ((_x!=_cap)&&(_x!=_coada)&&(_x!=_coada_01)&&!(surfaceIsWater getPos _x)) then {
                    if (_x isKindOf "landvehicle") then {
                        _x setvelocity [1+random 3, 1+random 3, 5+random 17];
                        ...
                        if (typeOf _x != "VirtualCurator_F") then {
                            [_x, 0.6, _bodyPart, _dmgtype] call ace_medical_fnc_adddamagetoUnit;
                        } else {};
                    };
                };
