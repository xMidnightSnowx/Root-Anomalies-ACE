...
if (isNil "_marker_farmer") exitwith {
    // ["Error! Farmer Marker not found! [MAIN/21]"] call zen_common_fnc_showMessage;
    ["Error", "Farmer Marker not found!", 10] call BIS_fnc_curatorHint;
    ...
    } do
    {
        ...
        if ((!alive _tgt_farmer)or(_tgt_farmer distance getmarkerPos _marker_farmer > _territory)) then {
            ...
            if !(count _list_unit_range_farm isEqualto 0) then {
                _tgt_farmer = selectRandom _list_unit_range_farm
            } else {
                _tgt_farmer = nil
            };
        };
