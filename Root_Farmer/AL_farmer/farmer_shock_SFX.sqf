...
playSound3D ["\Root_Anomalies\Root_Farmer\sound\explozie_3.ogg", "", false, position _farmer, 20, 5, 0];
	...
	if ((player distance _farmer)<_i) then 
				{
					...
					_dmgType = selectRandom ["backblast", "bite", "bullet", "explosive", "falling", "grenade", "punch", "ropeburn", "shell", "stab"];
					if !( player isKindOf "VirtualCurator_F") then { [player, _damage_farmer, _bodyPart, "falling"] call ace_medical_fnc_addDamageToUnit; };
				};
	sleep 0.05;
};
player allowdamage true;
